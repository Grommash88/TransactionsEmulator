...
public class Bank {
  ...
  private final double fraudPercent;
  ...
  private boolean isZeroAmount(long amount) {
    if (amount == 0) {
      Log.debug(LogMsg.ZERO_AMOUNT_TRANSFER.getMsg());
      return true;
    }
    return false;
  }
  ...
  private synchronized boolean isNotEnoughMoney(Account account, long amount) {
    if (amount > account.getMoney()) {
      Log.debug(LogMsg.NOT_ENOUGH_MONEY.getMsg());
      return true;
    }
    return false;
  }
  ...
  private boolean isSameAccount(Account from, Account to) {
    if (from == to) {
      Log.debug(LogMsg.SAME_ACCOUNTS.getMsg());
  ...
  private synchronized boolean isAccountBlocked(Account account) {
    if (account.isBlocked()) {
      Log.debug(LogMsg.BLOCKED_ACCOUNTS.getMsg());
      if (account.isBlocked()) {
  ...
  private void extFraudCheck(Account account) {
    synchronized (extFraudCheckLock) {
      ...
      Log.debug(String.format(LogMsg.CHECK_START.getMsg(), account.getNumber()));
      ...
      Log.debug(String.format(LogMsg.CHECK_FINISH.getMsg(), account.getNumber()));
      Log.timerFinish(System.currentTimeMillis() - startTime);
    }
  }
  ...
  public synchronized BigInteger getBankBalance() {
    ...
    Log.debug(String.format(LogMsg.ACCOUNT_LIST_PATTERN.getMsg(), this));
    ...
    Log.info(String.format(LogMsg.BALANCE_PATTERN.getMsg(), bankBalance));
